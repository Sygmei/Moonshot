...
local function getRampRadius(x, y, vecUnit, inOrbit)
    local dist = math.sqrt((x - center.x) * (x - center.x) + (y - center.y) * (y - center.y))
    if inOrbit then
        return (dist * dist - ORBIT_RADIUS * ORBIT_RADIUS) /
                   (2 * (ORBIT_RADIUS - getDistFromLine(x, y, vecUnit)))
    else
        return (dist * dist - ORBIT_RADIUS * ORBIT_RADIUS) /
                   (2 * (getDistFromLine(x, y, vecUnit) - ORBIT_RADIUS))
    end
end
...
local function getPotentialRampCircle(x, y, vecUnit, direction, inOrbit)
    ...
    return {
        radius = radius,
        x = x + vecPerpendicular.x * radius,
        y = y + vecPerpendicular.y * radius
    }
end
...
local function nbIntersections(line)
    ...
    if line.a == nil then
        ...
        delta = ORBIT_RADIUS * ORBIT_RADIUS * (1 + line.a * line.a) -
                    (center.y - line.a * center.x - line.b) *
                    (center.y - line.a * center.x - line.b)
    end
    if delta < 0 then
        return 0
